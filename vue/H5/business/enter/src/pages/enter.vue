		...
		<div class="page_hide" v-if="status==1">
			...
			<div class="margin">
				...
				<div class="type">
					...
					<div class="print_box">
						<!-- <input class="print" v-model="name" placeholder="输入推广员真实姓名" type="" name=""> -->
						<span class="print_text">{{secondMerchantName}}</span>
					</div>
				</div>
				...
				<div class="type">
					...
					<div class="print_box">
						<p class="print_text">彩票{{ticketRate}}%</p>
						<p class="print_text">游戏{{gameRate}}%</p>
					</div>
				</div>
			</div>
			...
			<div class="footer_text">
				选择立即申请入驻,即表示您同意接受以上邀请者及分润比例，并同意
				<router-link to="/settleInProtocol">
					<span class="content_tips">《中体华彩商户入驻协议》</span>
				</router-link>
			</div>
		</div>
...
<script>
	export default {
		data(){
			return {
				// ?secondMerchantID=4&createTime=1528958398196&ticketRate=3.5&gameRate=3.5&secondMerchantName=北京_朝阳区代理商
				status:1,
		        name:'',
		        mobile:'',
		        smsCode:'',
		        ...
		        ticketRate: this.$route.query.ticketRate,
		        gameRate: this.$route.query.gameRate,
		        createTime: this.$route.query.createTime,
		        secondMerchantName: this.$route.query.secondMerchantName,
			}
		},
		watch:{
			sheng(){
		...
		methods:{
			...
			getProvince(){
				let _this = this;
				_this.$http.get('/merApi/dict/code/list',{
					params:{
						code: 'province',
					}
				}).then((res)=>{
			...
			getCity(){
				let _this = this;
				_this.$http.get('/merApi/dict/parId/list',{
					params:{
						parId: this.sheng,
					}
				}).then((res)=>{
			...
			getCode(){
				...
				if(!this.send){
					...
					this.$http.get("/merApi/merchant/code",{
						params:{
						mobile:this.mobile,
						type:1,
						}
			...
			submit(){
				...
				this.btn1 = false;
				this.btn2 = true;
				_this.$http.post('/merApi/merchant/create/settled',formData).then((res)=>{
					console.log(res.data);
					if(res.data.status == 1){
			...
			initData(){
				let _this = this;
				_this.$http.get('/merApi/merchant/inviteUrl/isUse',{
					params:{
